...
for FILE in *_fwd_fixed.fastq; do BASENAME="${FILE%_fwd_fixed.fastq}"; kaiju -E 10-5 -v -z 16 -t ~/databases/kaijudb/nodes.dmp -f ~/databases/kaijudb/viruses/kaiju_db_viruses.fmi -i $FILE -j "${BASENAME}_rev_fixed.fastq" -o ~/Cracas_viroma/kaiju_done/$BASENAME.kaiju; done
